...
| **没黑窗口** | 默认弹独立程序窗口。排错时加 `--browser` 参数走浏览器模式 |
| **启动慢 1~2 秒** | 正常，要先起本地服务再开窗 |
...
**双击后没反应？**
先等 3 秒（要起服务）。还不行就看 exe 旁边有没有 `warehouse.log`，
里面写了崩溃原因。也可以用 `仓库管理系统.exe --browser` 试试浏览器模式。

**独立窗口打不开，只有浏览器？**
说明 pywebview 在你的电脑上不可用（多半是缺 Edge WebView2 运行时）。
装一下：https://developer.microsoft.com/microsoft-edge/webview2/
程序会自动降级到浏览器，功能不受影响。
