# -*- coding: utf-8 -*-
"""工作表 —— 稀疏单元格字典 + 列宽行高 + 冻结 + 合并 + 筛选 + 条件格式

重算策略：惰性 DFS + 记忆化。要哪格算哪格，算过的记下来；
碰到自己依赖自己（A1=A1+1）返回 #CIRC!，不无限递归；
改一格只清它下游的缓存，不整表重算。

安全：公式只走自写的词法/语法分析器 + 白名单函数表，
...
from ..kernel import to_num, to_text, to_bool, is_err
from ..kernel import lexer as LX
from ..kernel import parser as P
from .eval import make_evaluator        # 公式求值层（可插拔）
from .cell import Cell, MAX_CELLS
